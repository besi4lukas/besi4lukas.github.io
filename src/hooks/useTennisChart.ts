'use client';

import { useEffect, RefObject } from 'react';
import * as d3 from 'd3';
import { TennisMatchData } from '@/types';

export function useTennisChart(
  containerRef: RefObject<HTMLDivElement | null>,
  data: TennisMatchData[],
  winner: string,
  year: string
) {
  useEffect(() => {
    if (!containerRef.current || !data || data.length === 0) return;

    // Clear existing SVG
    d3.select(containerRef.current).selectAll('svg').remove();

    const container = containerRef.current;
    const svg = d3.select(container).append('svg');

    const margin = 80;
    const width = 1000 - 2 * margin;
    const height = 600 - 2 * margin;

    const chart = svg.append('g').attr('transform', `translate(${margin}, ${margin})`);

    const xScale = d3
      .scaleBand()
      .range([0, width])
      .domain(data.map((s) => s.player2))
      .padding(0.4);

    const yScale = d3.scaleLinear().range([height, 0]).domain([0, 100]);

    const makeXLines = () => d3.axisBottom(xScale);
    const makeYLines = () => d3.axisLeft(yScale);

    chart.append('g').attr('transform', `translate(0, ${height})`).call(d3.axisBottom(xScale));

    chart.append('g').call(d3.axisLeft(yScale));

    // Vertical grid lines
    chart
      .append('g')
      .attr('class', 'grid')
      .attr('transform', `translate(0, ${height})`)
      .call(makeXLines().tickSize(-height).tickFormat(() => ''));

    chart
      .append('g')
      .attr('class', 'grid')
      .call(makeYLines().tickSize(-width).tickFormat(() => ''));

    const barGroups = chart.selectAll('.bar-group').data(data).enter().append('g');

    barGroups
      .append('rect')
      .attr('class', 'bar')
      .attr('x', (g) => xScale(g.player2) || 0)
      .attr('y', (g) => yScale(g.break1))
      .attr('height', (g) => height - yScale(g.break1))
      .attr('width', xScale.bandwidth())
      .on('mouseenter', function (event, actual) {
        d3.selectAll('.value').attr('opacity', 0);

        d3.select(this)
          .transition()
          .duration(300)
          .attr('opacity', 0.6)
          .attr('x', (xScale(actual.player2) || 0) - 5)
          .attr('width', xScale.bandwidth() + 10);

        const y = yScale(actual.break1);

        chart
          .append('line')
          .attr('id', 'limit')
          .attr('x1', 0)
          .attr('y1', y)
          .attr('x2', width)
          .attr('y2', y);

        barGroups
          .append('text')
          .attr('class', 'divergence')
          .attr('x', (a) => (xScale(a.player2) || 0) + xScale.bandwidth() / 2)
          .attr('y', (a) => yScale(a.break1) + 30)
          .attr('fill', 'white')
          .attr('text-anchor', 'middle')
          .text((a) => {
            const divergence = (a.break1 - actual.break1).toFixed(1);
            if (a.player2 === actual.player2) return '';
            let text = '';
            if (parseFloat(divergence) > 0) text += '+';
            text += `${divergence}%`;
            return text;
          });
      })
      .on('mouseleave', function () {
        d3.selectAll('.value').attr('opacity', 1);

        d3.select(this)
          .transition()
          .duration(300)
          .attr('opacity', 1)
          .attr('x', (d) => xScale((d as TennisMatchData).player2) || 0)
          .attr('width', xScale.bandwidth());

        chart.selectAll('#limit').remove();
        chart.selectAll('.divergence').remove();
      });

    barGroups
      .append('text')
      .attr('class', 'value')
      .attr('x', (a) => (xScale(a.player2) || 0) + xScale.bandwidth() / 2)
      .attr('y', (a) => yScale(a.break1) + 30)
      .attr('text-anchor', 'middle')
      .text((a) => `${a.break1}%`);

    svg
      .append('text')
      .attr('class', 'label')
      .attr('x', -(height / 2) - margin)
      .attr('y', margin / 2.4)
      .attr('transform', 'rotate(-90)')
      .attr('text-anchor', 'middle')
      .text('Break (%)');

    svg
      .append('text')
      .attr('class', 'label')
      .attr('x', width / 2 + margin)
      .attr('y', height + margin * 1.7)
      .attr('text-anchor', 'middle')
      .text('losers');

    svg
      .append('text')
      .attr('class', 'title')
      .attr('x', width / 2 + margin)
      .attr('y', 40)
      .attr('text-anchor', 'middle')
      .text(`${winner}'s break points against losers in ${year}`);

    return () => {
      d3.select(container).selectAll('svg').remove();
    };
  }, [containerRef, data, winner, year]);
}
